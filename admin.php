				...
				<div class="row">
					...
					<?php 
							if(isset($_POST['title']) && isset($_FILES['desc']['tmp_name']) &&isset($_FILES['author_desc']['tmp_name']) && isset($_POST['content']) && isset($_POST['written_by']) && isset($_POST['author_content'])){
								...
								//$image_path = mkdir("../pedagoge_content/" . $date_compres , 0700)."/$desc_name";
								...
								if(!empty($title) && !empty($desc) && !empty($content) && !empty($written_by) && !empty($author_content)){
									...
									$query = "INSERT INTO `pedagoge_blog` VALUES('' , '$title' ,'$path$desc_name','$content','$date.$time','$written_by','$path$author_desc_name','$author_content','$counter','$counter')";
										...
										if($query_run = mysql_query($query)){
											...
											ob_end_flush();
												...
												echo '<h5>'.'SOMETHING WRONG WITH THE DATABASE'.'</h5>';
											}
									}else{
				...
				<div class="row">
					...
					<div class="col-md-6">
						<div class="form-group">
							...
							<input type="file" id="file" name="desc" required>
			...
			<div class="row">
				<div class="col-md-12">
					<div class="form-group">
						...
						<input type="text" name="written_by" class="form-control" id="written" placeholder="Written By : " required>
					</div>
				</div>
			</div>
		</div>
			...
			<div class="row">
				<div class="col-md-12">
					<div class="form-group">
						...
						<input type="text" name="author_content" maxlength="300" class="form-control" id="author_content" placeholder="Author Desc: " required>
					</div>
				</div>
			</div>
		</div>
...
//scripts for image type checking 
 $('#button').click(function() {
      var file = $('#file').val();
 ...
 EasyEditor.prototype.image = function(){
    ...
    jQuery(document).ready(function($) {
        var easyEditor = new EasyEditor('#editor', {
            buttons: ['bold', 'italic', 'link', 'image' ,'x' , 'source', 'h1 , h2' , 'h3' , 'h4' ,  'h5', 'alignleft','youtube',  'aligncenter', 'alignright', 'quote', 'code']
        ...
        $('.easyeditor-modal-content-body').find('form').ajaxForm({
            ...
            complete: function(get) {
                if(get.responseText != 'null') {
					...
					easyEditor.insertHtml('<center><figure><img src="pedagoge_content/'+ get.responseText +'" alt="" class="img-responsive"></figure></center>');
